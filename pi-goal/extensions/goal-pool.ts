...
export function goalSelectorLabel(goal: GoalRecord, focusedGoalId: string | null): string {
	...
	return `${marker} ${goal.id} | ${statusLabel(goal)} | ${mode} | ${truncateText(displayObjectiveTitle(goal.objective), 72)}`;
}
...
export function buildUnfocusedOpenGoalsSummary(openGoalCount: number): string {
	return `No goal is focused in this session. ${openGoalCount} open goal${openGoalCount === 1 ? "" : "s"} exist in session storage. Use /goal-focus to choose the session focus before doing goal work.`;
}
