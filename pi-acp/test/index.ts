...
async function runTests() {
	...
	// Test 25: session/list returns sessions array
	const t25Result = await new Promise<boolean>(async (resolve) => {
		const testAgent = spawn("pi", ["--acp"], { stdio: ["pipe", "pipe", "pipe"] });
		const pending = new Map<number | string, (msg: JsonRpcMessage) => void>();
		testAgent.stdout?.on("data", (data) => {
			for (const line of data.toString().split("\n")) {
		...
		const listRes = await send({ jsonrpc: "2.0", id: 2, method: "session/list", params: {} });
		testAgent.kill();
		if (listRes.result && Array.isArray((listRes.result as any).sessions)) {
			resolve(true);
		} else {
			resolve(false);
		}
