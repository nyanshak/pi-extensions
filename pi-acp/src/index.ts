...
class AcpProtocolHandler {
		...
		try {
			switch (method) {
				...
				case "session/request_permission":
					...
					break;
				case "session/list":
					this.handleSessionList(params as any, requestId);
					break;
				case "session/update_metadata":
					this.handleSessionUpdateMetadata(params as any, requestId);
					break;
				default:
	...
	private handleSessionList(params: {
		cursor?: string;
		pageSize?: number;
		_meta?: Record<string, unknown>;
	}, id: number | string | undefined): void {
		...
		let startIndex = 0;
		if (params.cursor) {
			const cursorIndex = allSessions.findIndex(s => s.id === params.cursor);
			if (cursorIndex >= 0) {
		...
		// Build response
		const response: {
			sessions: { sessionId: string; name?: string; lastActive?: string }[];
			nextCursor?: string;
		} = {
		...
		// Add next cursor if there are more sessions
		if (endIndex < allSessions.length) {
			response.nextCursor = sessions[sessions.length - 1]?.id;
		}
	...
	private handleSessionUpdateMetadata(params: {
		sessionId: string;
		name?: string;
		description?: string;
		_meta?: Record<string, unknown>;
		...
		// Update metadata fields (all fields are optional per spec)
		if (params.name !== undefined) {
			session.name = params.name;
		}
		if (params.description !== undefined) {
			(session as any).description = params.description;
		}
